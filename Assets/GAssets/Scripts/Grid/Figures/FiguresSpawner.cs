...
namespace Core.Grid.Figures
{
    public class FiguresSpawner : MonoBehaviour, ISavable
    {
        ...
        {
            FindObjectOfType<RandomNumbersGenerator.RandomNumbersGenerator>().IncreaseRandomCounter();
        ...
        {
            RandomNumbersGenerator.RandomNumbersGenerator randomNumbersGenerator = FindObjectOfType<RandomNumbersGenerator.RandomNumbersGenerator>();
            for (_currentFiguresCount = 0; _currentFiguresCount < _desiredFiguresCount; _currentFiguresCount++)
            {
                ...
                figure.Initialize(_eventBus, _figuresHolder, _grid,
                    _colorsChanger.GetFiguresColors().Figures[figureDragHandler.FigureName], figureId);
        ...
        public async void SpawnReviveFigures(int figureIndex = 3)
        {
            ...
            for (_currentFiguresCount = 0; _currentFiguresCount < _desiredFiguresCount; _currentFiguresCount++)
            {
                ...
                var figure = Instantiate(figureDragHandler, _spawnPoint.position, Quaternion.identity);
                figure.Initialize(_eventBus, _figuresHolder, _grid,
                    _colorsChanger.GetFiguresColors().Figures[figureDragHandler.FigureName], figureIndex);
                _figuresHolder.AddFigure(figure);
            }
