...
namespace Core.Grid
{
    public class Grid : MonoBehaviour, IProvidable, ISavable
    {
        ...
        public bool TryPlaceFigure(FigureDragHandler figureDragHandler, GridCell closestCell)
        {
            ...
            if (!CanPlaceFigure(figureDragHandler.Shape, (int) closestCell.GridPos.x, (int) closestCell.GridPos.y))
                return false;

            List<Vector2> touchedGridCellsPos = new List<Vector2>();
            foreach (var pos in figureDragHandler.FigureData.Keys)
            {
                GridCell gridCell = GridCells[(int) closestCell.GridPos.x + (int) pos.x,
                    (int) closestCell.GridPos.y + (int) pos.y];
        ...
        public async void CheckLinesToDelete(List<Vector2> touchedCells)
        {
            ...
            if (linesToDelete > 0)
            {
                ...
                _eventBus.Publish<(int, Vector3)>(BusEventType.AddMoney,
                    (linesToDelete,
                        GridCells[(int) touchedCells[0].x, (int) touchedCells[0].y].Cell.transform.position));
            }
        ...
        private void LoadSavedGrid(GridSaveData data)
        {
            ...
            for (int y = 0; y < data.GridCells.GetLength(1); y++)
            {
                for (int x = 0; x < data.GridCells.GetLength(0); x++)
                {
                    ...
                    {
                        Cell newCell = Instantiate(_cellPrefab, GridCells[x, y].transform.position, Quaternion.identity)
                            .GetComponent<Cell>();
